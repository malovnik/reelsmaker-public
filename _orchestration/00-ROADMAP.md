...
| 13 | Security-скан секретов → private→public | 2 swarm | история вычищена, PUBLIC | ✅ DONE |

## ⚠️ ОБЯЗАТЕЛЬНОЕ ДЕЙСТВИЕ ПОЛЬЗОВАТЕЛЯ
**Ротировать Zhipu API-ключ** на open.bigmodel.cn — он был в истории (теперь вычищен через filter-branch, в репо его нет), но считается скомпрометированным с момента существования. Старый ключ `f639…` отозвать, новый положить в локальный `.env`.

## Phase 14-15 (после публикации): лаунчеры + README
| # | Фаза | Агентов | Статус |
|---|------|---------|--------|
| 14 | Кросс-платформенные лаунчеры (Win10+/Mac/Linux) + STT cross-platform unblock | recon 3×3 → dev 3 → валид 3 → доводка → ре-валид 2 | ✅ DONE (GO ×3 ОС) |
| 15 | README RU+EN + валидация «не пиздит» 3 агента | 3 | ✅ DONE (честный, неточности исправлены) |

## ИТОГ
Все 13 фаз пройдены. Сервис production-ready, опубликован: https://github.com/malovnik/reelsmaker-public
~50+ агентов в роях, двойная валидация каждой фазы, 3 финальных цикла ALL GO. NO MOCKS.
